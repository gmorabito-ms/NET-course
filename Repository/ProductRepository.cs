...
class ProductRepository : IProductRepository
{
    ...
    public bool Save()
    {
        return _db.SaveChanges() >= 0; // this alreadey returns true or false
    }

    public ICollection<Product> SearchProducts(string searchTerm)
    {
        IQueryable<Product> query = _db.Products;
        var searchTermToLower = searchTerm.ToLower().Trim();
        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            query = query.Include(p => p.Category)
            .Where(
            p => p.Name.ToLower().Trim().Contains(searchTermToLower) ||
            p.Description.ToLower().Trim().Contains(searchTermToLower));
        }
